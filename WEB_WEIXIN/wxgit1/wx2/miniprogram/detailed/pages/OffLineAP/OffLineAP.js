...
Page({
  ...
  CompanyYes() {
    ...
    if (this.data.CompanyVal == '上海福满家') {
      this.setData({
        companyName: '全家'
      })
    } else if (this.data.CompanyVal == '集享贝瑞') {
      this.setData({
        companyName: '贝瑞'
      })
    }
  ...
  onDetailsIp(e) {
    ...
    wx.navigateTo({
      url: '../../../detailed/pages/DetailsIp/DetailsIp?DetailsAp=' + JSON.stringify(e.currentTarget.dataset.item),
    })
  },
  ...
  onLoad: function (options) {
    if (options.hasOwnProperty('companyName')) {
      ...
      // 更改题头
      wx.setNavigationBarTitle({
        title: this.data.companyName + 'AP查询'
      })
      this.IpQueryRegion();
    } else {
      this.onCompanyShow();
      ...
      .then(res => {
        ...
        if (myData !== null) {
          ...
          myData.forEach(function (value) {
            ...
            value.ShopNames = value.ShopName.slice(0, 7) + '..'
            else
            value.ShopNames = value.ShopName
