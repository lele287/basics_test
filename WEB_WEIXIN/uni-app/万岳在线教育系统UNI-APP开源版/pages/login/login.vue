<template>
	<view class="login-all-wrap">
		...
		<view class="flex align-center justify-center login-botttom-text font-sm">
			登录即代表你已同意<text class="xieyi font-sm">《用户协议》</text>
		</view>
	...
	export default {
		...
		methods: {
			...
			wechatlogin() {
							...
							success: (info) => {
								...
								uni.request({
									...
									success: function(res2) {
										...
										that.thirdlogin(that.unionid, that.nickName, that.avatar);
				...
				// #ifndef MP-WEIXIN
				uni.showToast({
					title: '未配置appid，请先使用手机号登录',
					icon: 'none'
				});
				// #endif
			},
			thirdlogin(unionid, nickName, avatar) {
				uni.showLoading({
					title: '加载中',
					mask: false
				});
