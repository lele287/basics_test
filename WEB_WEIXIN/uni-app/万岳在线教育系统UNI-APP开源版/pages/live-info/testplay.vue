...
<template>
	<view class="live-all-wrap">
		...
		<view style="width: 750px; height: 90p; flex-direction: row; background-color: #FFFFFF;">
			<view style="height: 90px;width: 250px; align-items: center;" v-for="(item,index) in tabBars" :key="index" @click="changeXXK(index)"
			 :ref="`tab${index}`">
				<text style="font-size: 30px; height: 90px; width: 100px;text-align: center; line-height: 90px; border-bottom-width: 5px; border-style: solid;"
				 :style="{'color':tabIndex ===index?'#3BDCA8':'#A9A5A0','border-color':tabIndex === index? '#3BDCA8':'#FFFFFF'}">{{item.name}}</text>
			</view>

		</view>

		<view :current="tabIndex" @change="onChangeTab" :style="{height : scrollH + 'px'}" style="width: 750px;background-color: #F4F5F6;">
			<view v-if="tabIndex ==0" class="zhinan">
				<text class="zhinantext">听课指南</text>
			</view>
			<view v-if="tabIndex ==0">
				<scroll-view :style="{height : scrollH+ 'px'}" class="scroll1" scroll-y="true" :scroll-into-view="scrollInto"
					...
					<block v-for="(item,index) in list" :key="index">
						...
						<template v-if="item.uid === myID">
							<view :id="'chat' +index" class="messagebottom2">
								...
								<text v-if="item.msgtype == '1'" class="wen">问</text>
							</view>
						</template>
						<template v-else>
							<view :id="'chat' +index" class="messagebottom">
								...
								<text v-if="item.msgtype == '1'" class="wen2">问</text>
							</view>
			...
			<view v-if="tabIndex ==2">
				<scroll-view :style="{height : scrollH + 100 + 'px'}" class="scroll1" scroll-y="true" :scroll-into-view="scrollInto"
				 scroll-with-animation style="position: absolute;left:0; top: 0; 
				    right: 0; bottom: 10rpx; ">
					<block v-for="(item,index) in list" :key="index">
						<template v-if="item.uid === myID">
							<view v-if="item.msgtype == '1'" :id="'chat' +index" class="messagebottom2">
								<image :src="item.avatar" class="rounded"></image>
								<view class="message">
									<text class="textmessage">{{item.content}}</text>
								</view>
						...
						<template v-else>
							<view v-if="item.msgtype == '1'" :id="'chat' +index" class="messagebottom">
								<image :src="item.avatar" class="rounded"></image>
								<view class="message">
									<text class="user_nickname">{{item.user_nickname}}</text>
									<text class="textmessage">{{item.content}}</text>
...
<script>
	...
	export default {
		...
		onReady() {
			...
			uni.getSystemInfo({
				success: (res) => {
					this.scrollH = res.screenHeight - 120;
				}
			})
			this.pageToBottom();
		},
