...
<template>
	<view class="live-all-wrap">
		...
		<view style="width: 100%; height: 90rpx; flex-direction: row; background-color: #FFFFFF;">
			<view style="height: 90rpx;width: 33%; align-items: center;" v-for="(item,index) in tabBars" :key="index" @click="changeXXK(index)"
			 :ref="`tab${index}`">
		...
		<view class="swiper-box" :current="tabIndex" @change="onChangeTab" :style="{height : scrollH + 'rpx'}" style="width: 750rpx;background-color: #F4F5F6;">
			...
			<view v-if="tabIndex ==0">
				...
				<scroll-view :style="{height : scrollH+ 'rpx'}" class="scroll1" scroll-y="true" :scroll-into-view="scrollInto"
				 scroll-with-animation style="position: absolute;left:0; top: 0; 
			        right: 0; bottom: 10rrpx; background-color: #007AFF;">
			...
			<view class="inputbottom">
				<view class="inputclass">
					...
					<view class="inputborder">
						<input ref="input1" @input='inputing' type="text" confirm-type="send" v-model="content" class="input"
						 placeholder="说点什么..." />
		...
		onReady() {
			...
			uni.getSystemInfo({
				success: (res) => {
					...
					this.scrollH = res.screenHeight - 160;
				}
			})
			this.pageToBottom();
		},
